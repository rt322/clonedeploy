...
const Hotel = ({ e }) => {
  return (
    <div className="border-2 border-red-500 rounded-lg lg:h-95 w-full lg:m-5 m-8">
      <div className="flex">
        <Image src={e?.banner} alt="hotel banner" width={200} height={200} className="w-96 h-60 mr-3" />

        <div className="grid grid-rows-3">
          {e
            ? e.gallery?.map((ele) => {
                return <Image key={ele} src={ele} alt="hotel gallery" width={200} height={200} className="lg:w-28" />;
              })
            : ""}
        </div>
                ...
                ? e.facilities?.map((ele) => {
                    return (
                      <li key={ele.name} className="mr-10 mb-3">
                        <span className="m-3">
                          <Image src={ele.img} alt={ele.name} width={200} height={200} className="w-20 h-20 rounded-full" />
                        </span>
